...
public class ExerciseRepository {
    ...
    public int insert(Exercise exercise) {
        return jdbcTemplate.update("INSERT INTO exercises (id, name, sets, reps, weight, workout) VALUES (?, ?, ?, ?, ?, ?)", 
        exercise.getId(), exercise.getName(), exercise.getSets(), exercise.getReps(), 
        exercise.getWeight(), exercise.getWorkout());
    }

    public int update(Exercise exercise) {
        return jdbcTemplate.update(
            "UPDATE exercises SET name = ?, sets = ?, reps = ?, weight = ?, workout = ? WHERE id = ?",
            exercise.getName(), exercise.getSets(), exercise.getReps(), exercise.getWeight(),
            exercise.getWorkout(), exercise.getId());
    }
}
